...
function(value){
    if(value == 'edition'){
        ...
        con = gaeas_cradle("***REMOVED***@gmail.com")
...
get_input_from_user_input <-
function(data= data_list[[1]], input = user_input){
...
get_user_targeted_data <-
function(data=data_list[[1]],needed_input=acquired_user_input,  from = NULL,to = NULL){
    if(is.null(from) ){
...
get_user_targeted_field <-
function(data = user_selected_base_data, column=user_input_field_of_interest, mavg_short=7, mavg_long=30){
...
framing_for_graph <-
function(data = fundamental_input_selected_tbl, needed_input = user_input_field_of_interest){
...
generate_commentary <-
function(data = fundamental_input_selected_tbl){
    warning_signal = data %>%
        tail(1) %>%
