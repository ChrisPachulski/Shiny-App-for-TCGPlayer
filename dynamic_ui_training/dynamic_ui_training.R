...
ui <- fixedPage(
    ...
    div(
        ...
        column(
            width=4,
            class="well",
            selectInput(inputId = 'sales_metric_1',
                        label   ='Sales Metric',
    ...
    div(
        ...
        column(
            ...
            numericInput(inputId = 'metric_value_2',
                         label   ='Metric Percent',
                         value   = 0.5),
            actionButton(inputId = 'add_card', label='Add'),
    ...
    sales_card = eventReactive(input$create_card_1,{
        column(
            ...
            info_card(
                ...
                sub_icon = ifelse(input$metric_value_1 < 0, 'arrow-down', 'arrow-up'),
                sub_text_color = ifelse(input$metric_value_1 < 0, 'danger', 'success')
            )
        )
    })
                ...
                map(.f=function(data){
                    column(
                        ...
                        info_card(
                            ...
                            sub_icon = ifelse(data$metric_value < 0, 'arrow-down', 'arrow-up'),
                            sub_text_color = ifelse(data$metric_value < 0, 'danger', 'success')
                        )
                    )
                }) %>% 
                tagList()
